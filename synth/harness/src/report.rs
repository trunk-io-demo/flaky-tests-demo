...
mod tests {
    ...
    fn spec_with(outcomes: &[(&str, &str, Outcome)]) -> ReportSpec {
        ...
        for (suite, name, outcome) in outcomes {
            spec.push(TestCaseSpec::new(
                TestIdentity::in_class("demo", suite, "Class00", name),
                *outcome,
    ...
    fn identity_fields_reach_the_xml_unchanged() {
        ...
        assert!(
            xml.contains(r#"classname="synth.demo.CheckoutFlow.Class00""#),
            "{xml}"
        );
        assert!(
            xml.contains(r#"file="synth/demo/checkout_flow/class00.ts""#),
            "{xml}"
        );
    ...
    fn retry_duration_pins_failed_attempts_independently_of_the_pass() {
        ...
        spec.push(
            TestCaseSpec::new(
                TestIdentity::in_class("demo", "Alpha", "Class00", "one"),
    ...
    fn the_run_is_laid_out_to_end_at_finished_at() {
        ...
        for name in ["one", "two", "three"] {
            spec.push(TestCaseSpec::new(
                TestIdentity::in_class("demo", "Alpha", "Class00", name),
                Outcome::Pass,
